...
app.post("/Complaint/:id", async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      ...
      You can track its status inside your dashboard.
...
app.post("/assignedComplaints", async (req, res) => {
  ...
  try {
    ...
    const assigned = await AssignedComplaint.create(assignedComplaint);

    // 📧 EMAIL: Complaint Assigned
    const complaint = await ComplaintSchema.findById(assignedComplaint.complaintId);
    const complaintUser = await UserSchema.findById(complaint.userId);

    await sendEmail(
      complaintUser.email,
      "Complaint Assigned",
      `Hello ${complaintUser.name},

    Your complaint has been assigned to an agent.
...
app.put("/complaint/:complaintId", async (req, res) => {
  try {
    ...
    const complaintUser = await UserSchema.findById(updatedComplaint.userId);
